    ...
    public class MapStringCheckHelper
    {
        ...
        public static bool CheckMapString(FieldInfo fieldInfo, FieldCheckRule checkRule, out string errorString)
        {
            ...
            for (int i = 0; i < checkRuleList.Length; ++i)
            {
                ...
                if (oneCheckRule.Equals(IF_CONDITION_START_STRING, StringComparison.CurrentCultureIgnoreCase) ||
                    !oneCheckRule.StartsWith(IF_CONDITION_START_STRING, StringComparison.CurrentCultureIgnoreCase))
                {
                ...
                else
                {
                    ...
                    for (int j = 0; j < ifConditionStringList.Length; j++)
                    {
                        ...
                        FieldInfo targetFieldInfo =
                            TableCheckHelper.GetFieldByIndexDefineString(fieldName, AppValues.TableInfo[fieldInfo.TableName], out errorString);
                        if (errorString != null)
                        {
                            errorString = string.Format("mapString型的内容检查规则声明错误，无法根据索引字符串\"{0}\"在表格{1}找到要对应的字段，错误信息为：{2}\n", fieldName,
                                fieldInfo.TableName, errorString);
                            return false;
                        }

                        // 检查字段类型是否符合要求
                        if (targetFieldInfo.DataType != DataType.Int && targetFieldInfo.DataType != DataType.Long &&
                            targetFieldInfo.DataType != DataType.Float && targetFieldInfo.DataType != DataType.Bool &&
                            targetFieldInfo.DataType != DataType.String)
                        {
                            errorString = string.Format(
                                "mapString型的内容检查规则声明错误，条件字段（{0}）的数据类型为{1}，而mapString型内容检查规则中，if条件字段只能为int、long、float、bool或string型\n", fieldName,
                        ...
                        if (condition.Relation != Relation.Equal &&
                            (targetFieldInfo.DataType == DataType.Bool || targetFieldInfo.DataType == DataType.String))
                        {
                            errorString = string.Format("mapString型的内容检查规则声明错误，条件字段（{0}）为{1}类型，只能进行等于判定，而你设置的关系符为{2}\n", fieldName,
                                targetFieldInfo.DataType, condition.Relation);
                            return false;
                        }
            ...
            foreach (var item in mapStringCheckRule.CheckRuleList)
            {
                ...
                if (emptyConditionRowIndex.Count > 0)
                {
                    string warningString = string.Format("警告：mapString型字段\"{0}\"（列号：{1}）的检查条件（{2}）中的字段，因为以下行中数据无效，视为不满足条件，不对对应行中的mapString进行检查：{3}\n",
                        fieldInfo.FieldName, Utils.GetExcelColumnName(fieldInfo.ColumnSeq + 1), checkRuleList[ruleIndex],
                        Utils.CombineString(emptyConditionRowIndex, ","));
                ...
                foreach (int index in targetDataIndex)
                {
                    ...
                    if (_CheckMapStringData(jsonData, requiredInfo, out errorString) == false)
                        oneConditionStringBuilder.AppendFormat("第{0}行填写的数据（{1}）：\n{2}\n", index + AppValues.DATA_FIELD_DATA_START_INDEX + 1,
                            fieldInfo.JsonString[index], errorString);
                }
                ...
                if (emptyDataRowIndex.Count > 0)
                {
                    string warningString = string.Format("警告：在对mapString型字段\"{0}\"（列号：{1}）执行检查条件（{2}）时，因为以下行中数据无效，跳过对mapString的检查：{3}\n",
                        fieldInfo.FieldName, Utils.GetExcelColumnName(fieldInfo.ColumnSeq + 1), checkRuleList[ruleIndex],
                        Utils.CombineString(emptyDataRowIndex, ","));
                    Utils.LogWarning(warningString);
                }
        ...
        private static bool _CheckMapStringParamData(JsonData jsonData, MapStringParamRequiredInfo paramRequiredInfo, out string errorString)
        {
            ...
            if (paramRequiredInfo.MapStringRequiredInfo == null)
            {
                ...
                {
                    if (((IDictionary) jsonData).Contains(paramRequiredInfo.ParamName) == false)
                ...
                {
                    if (((IDictionary) jsonData).Contains(paramRequiredInfo.ParamName) == true)
            ...
            else
            {
                if (paramRequiredInfo.ArrayParamRequiredInfo != null)
                {
                    ...
                    if (((IDictionary) jsonData).Contains(paramRequiredInfo.ParamName))
                ...
                else if (((IDictionary) jsonData).Contains(paramRequiredInfo.ParamName))
                {
    ...
    public class MapStringCheckRuleParser
    {
        ...
        private void _GetMapElement(MapStringRequiredInfo requiredInfo, MapStringInfo formatDefine, out string errorString)
        {
            ...
            else if (valueToken.TokenType == MapStringTokenType.StartMap)
            {
                ...
                {
                    errorString = string.Format("mapString定义中要求key名（{0}）对应{1}类型的数据，而输入的检查规则却以mapString型进行设置，请不要对非mapString型使用括号声明", key,
                        paramInfo.DataType);
                    return;
                }
            }
        ...
        private MapStringParamRequiredInfo _GetArrayElement(MapStringParamInfo paramInfo, out string errorString)
        {
            ...
            if (valueToken.TokenType == MapStringTokenType.Number)
            {
                ...
                {
                    errorString = string.Format("array层级{0}子元素对应的参数要求非法，输入值为\"{1}\"，请用1或0分别设置该参数为必须含有或不允许含有", paramInfo.ArrayDepth,
            ...
            else if (valueToken.TokenType == MapStringTokenType.StartMap)
            {
                ...
                {
                    errorString = string.Format("array层级{0}子元素要求对应{1}类型的数据，而输入的检查规则却以mapString型进行设置，请不要对非mapString型使用括号声明", paramInfo.ArrayDepth,
                        paramInfo.DataType);
                    return null;
                }
            }
            else if (valueToken.TokenType == MapStringTokenType.StartArray)
            {
                ...
                {
                    errorString = string.Format("array层级{0}子元素要求对应{1}类型的数据，而输入的检查规则却以array型进行设置，请不要对非array型使用[]声明", paramInfo.ArrayDepth,
                        paramInfo.DataType);
                    return null;
                }
            }

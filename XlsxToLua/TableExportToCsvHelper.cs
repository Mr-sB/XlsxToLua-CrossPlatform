    ...
    public class TableExportToCsvHelper
    {
        public static bool ExportTableToCsv(TableInfo tableInfo, out string errorString)
        {
            ...
            if (AppValues.ExportCsvIsExportColumnDataType == true)
            {
                ...
                // 去掉开头多加的一个分隔符
                rowContentList.Insert(AppValues.ExportCsvIsExportColumnName == true ? 1 : 0,
                    columnDataTypeStringBuilder.Remove(0, AppValues.ExportCsvSplitString.Length));
            }
            ...
            switch (fieldInfo.DataType)
            {
                ...
                case DataType.Date:
                {
                    DateFormatType dateFormatType =
                        TableAnalyzeHelper.GetDateFormatType(fieldInfo.ExtraParam[AppValues.TABLE_INFO_EXTRA_PARAM_KEY_DATE_TO_LUA_FORMAT]
                            .ToString());
                    ...
                    {
                        dateFormatType =
                            TableAnalyzeHelper.GetDateFormatType(fieldInfo.ExtraParam[AppValues.TABLE_INFO_EXTRA_PARAM_KEY_DATE_INPUT_FORMAT]
                                .ToString());
                ...
                case DataType.Time:
                {
                    TimeFormatType timeFormatType =
                        TableAnalyzeHelper.GetTimeFormatType(fieldInfo.ExtraParam[AppValues.TABLE_INFO_EXTRA_PARAM_KEY_TIME_TO_LUA_FORMAT]
                            .ToString());
                    switch (timeFormatType)
                    {
                        case TimeFormatType.FormatString:
                        {
                            for (int row = 0; row < rowCount; ++row)
                            {
                                ...
                                stringBuilder.Append(AppValues.ExportCsvSplitString);
                                if (fieldInfo.Data[row] != null)
                                    stringBuilder.Append(((DateTime) (fieldInfo.Data[row])).ToString(fieldInfo
                                        .ExtraParam[AppValues.TABLE_INFO_EXTRA_PARAM_KEY_TIME_TO_LUA_FORMAT].ToString()));
                            }

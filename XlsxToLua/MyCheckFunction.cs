    ...
    public static class MyCheckFunction
    {
        ...
        public static bool CheckRewardListField(FieldInfo fieldInfo, out string errorString)
        {
            ...
            foreach (FieldInfo childDictField in fieldInfo.ChildField)
            {
                ...
                {
                    errorString = string.Format("一个奖励项的dict必须由type、id、count三个int型字段组成，而你填写的奖励项由{0}个字段组成，出错的dict列号为{1}",
                        childDictField.ChildField.Count, Utils.GetExcelColumnName(childDictField.ColumnSeq + 1));
                    return false;
                }
                ...
                foreach (FieldInfo childBaseField in childDictField.ChildField)
                {
                    ...
                    {
                        errorString = string.Format("一个奖励项的dict必须由type、id、count三个int型字段组成，而你填写的奖励项中含有名为\"{0}\"的字段，出错的dict列号为{1}",
                            childBaseField.FieldName, Utils.GetExcelColumnName(childDictField.ColumnSeq + 1));
                        return false;
                    }
                    else if (childBaseField.DataType != DataType.Int)
                    {
                        errorString = string.Format("一个奖励项的dict必须由type、id、count三个int型字段组成，而你填写的奖励项中的{0}字段的数据类型为{1}，出错的dict列号为{2}",
                            childBaseField.DataType, childBaseField.FieldName, Utils.GetExcelColumnName(childDictField.ColumnSeq + 1));
                        return false;
                    }
                }
            }
                ...
                foreach (FieldInfo childDictField in fieldInfo.ChildField)
                {
                    ...
                    foreach (FieldInfo field in childDictField.ChildField)
                    {
                        if (field.FieldName.Equals("type", System.StringComparison.CurrentCultureIgnoreCase))
                        {
                            ...
                            {
                                errorString = string.Format("第{0}行第{1}列\"type\"字段填写值为空", i + AppValues.DATA_FIELD_DATA_START_INDEX + 1,
                                    Utils.GetExcelColumnName(field.ColumnSeq + 1));
                                return false;
                            }
                            else
                        ...
                        else if (field.FieldName.Equals("id", System.StringComparison.CurrentCultureIgnoreCase))
                        {
                            if (field.Data[i] == null)
                            {
                                errorString = string.Format("第{0}行第{1}列\"id\"字段填写值为空", i + AppValues.DATA_FIELD_DATA_START_INDEX + 1,
                                    Utils.GetExcelColumnName(field.ColumnSeq + 1));
                                return false;
                            }
                            else
                        ...
                        else if (field.FieldName.Equals("count", System.StringComparison.CurrentCultureIgnoreCase))
                        {
                            if (field.Data[i] == null)
                            {
                                errorString = string.Format("第{0}行第{1}列\"count\"字段填写值为空", i + AppValues.DATA_FIELD_DATA_START_INDEX + 1,
                                    Utils.GetExcelColumnName(field.ColumnSeq + 1));
                                return false;
                            }
                            else
                    ...
                    if (type == PROP_TYPE)
                    {
                        ...
                        if (!PROP_KEYS.Contains(id))
                            errorStringBuilder.AppendFormat("第{0}行第{1}列的奖励项中所填的奖励道具（id={2}）在道具表中不存在\n", i + AppValues.DATA_FIELD_DATA_START_INDEX + 1,
                                Utils.GetExcelColumnName(childDictField.ColumnSeq + 1), id);
                    }
                    ...
                    if (count < 1)
                        errorStringBuilder.AppendFormat("第{0}行第{1}列的奖励项中所填的奖励数量低于1\n", i + AppValues.DATA_FIELD_DATA_START_INDEX + 1,
                            Utils.GetExcelColumnName(childDictField.ColumnSeq + 1), type);
                }
            }
            ...
            for (int i = 0; i < dataCount; ++i)
            {
                ...
                if (seqInfo.ContainsKey(seq))
                    errorStringBuilder.AppendFormat("第{0}行与第{1}行完全重复\n", i + AppValues.DATA_FIELD_DATA_START_INDEX + 1,
                        seqInfo[seq] + AppValues.DATA_FIELD_DATA_START_INDEX + 1);
                else
                    seqInfo.Add(seq, i);
            }

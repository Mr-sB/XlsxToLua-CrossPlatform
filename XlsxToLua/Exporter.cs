            ...
            try
            {
                ...
                for (int i = 4; i < args.Length; ++i)
                {
                    ...
                    if (tempParam.Equals(AppValues.EXPORT_INCLUDE_SUBFOLDER_PARAM_STRING, StringComparison.CurrentCultureIgnoreCase))
                    {
                        ...
                        List<string> existExcelFilePaths = Utils.GetSupportedExcelFilePaths(AppValues.ExcelFolderPath, SearchOption.AllDirectories);
                ...
                {
                    List<string> existExcelFilePaths = Utils.GetSupportedExcelFilePaths(AppValues.ExcelFolderPath, SearchOption.TopDirectoryOnly);
                    foreach (string filePath in existExcelFilePaths)
                    {
                        string fileName = Path.GetFileNameWithoutExtension(filePath);
                        if (fileName.StartsWith(AppValues.EXCEL_TEMP_FILE_FILE_NAME_START_STRING))
                ...
                for (int i = 4; i < args.Length; ++i)
                {
                    ...
                    else if (param.Equals(AppValues.EXPORT_KEEP_DIRECTORY_STRUCTURE_PARAM_STRING, StringComparison.CurrentCultureIgnoreCase))
                    {
                        if (AppValues.IsExportIncludeSubfolder == false)
                        {
                            string warningString =
                                string.Format(
                                    "警告：只有通过设置{0}参数，将要导出的Excel文件夹下的各级子文件夹中的Excel文件也进行导出时，指定{1}参数设置将生成的文件按原Excel文件所在的目录结构进行存储才有意义，请检查是否遗漏声明{0}参数",
                                    AppValues.EXPORT_INCLUDE_SUBFOLDER_PARAM_STRING, AppValues.EXPORT_KEEP_DIRECTORY_STRUCTURE_PARAM_STRING);
                    ...
                    else if (param.StartsWith(AppValues.EXCEPT_EXPORT_PARAM_STRING, StringComparison.CurrentCultureIgnoreCase))
                    {
                        ...
                        else
                        {
                            ...
                            foreach (string exceptExportExcelFileName in AppValues.ExceptExportTableNames)
                            {
                                ...
                                {
                                    Utils.LogErrorAndExit(string.Format("设置要忽略导出的Excel文件（{0}）不存在，请检查后重试并注意区分大小写",
                                        Utils.CombinePath(AppValues.ExcelFolderPath, string.Concat(exceptExportExcelFileName, ".xlsx"))));
                                    return -1;
                                }
                            }
                    ...
                    else if (param.StartsWith(AppValues.PART_EXPORT_PARAM_STRING, StringComparison.CurrentCultureIgnoreCase))
                    {
                        ...
                        else
                        {
                            ...
                            foreach (string exportExcelFileName in tempExportTableName)
                            {
                                ...
                                {
                                    Utils.LogErrorAndExit(string.Format("要求导出的Excel文件（{0}）不存在，请检查后重试并注意区分大小写",
                                        Utils.CombinePath(AppValues.ExcelFolderPath, string.Concat(exportExcelFileName, ".xlsx"))));
                                    return -1;
                                }
                            }
                    ...
                    else if (param.StartsWith(AppValues.AUTO_NAME_CSV_CLASS_PARAM_STRING, StringComparison.CurrentCultureIgnoreCase))
                    {
                        ...
                        {
                            Utils.LogErrorAndExit(string.Format("错误：声明自动命名导出csv对应的C#或Java类名规则的参数{0}后必须在英文小括号内声明各个具体参数",
                                AppValues.AUTO_NAME_CSV_CLASS_PARAM_STRING));
                            return -1;
                        }
                        else
                        {
                            ...
                            if (string.IsNullOrEmpty(paramString))
                                Utils.LogWarning(string.Format(
                                    "警告：声明的{0}参数没有在小括号中一并声明下属的子参数{1}和{2}来配置自动命名导出csv对应的C#或Java类名时统一添加的前后缀，若不想设置，可以直接不配置此参数，而不是将参数值留空",
                                    AppValues.AUTO_NAME_CSV_CLASS_PARAM_STRING, AppValues.AUTO_NAME_CSV_CLASS_PARAM_CLASS_NAME_PREFIX_PARAM_STRING,
                            ...
                            else
                            {
                                ...
                                foreach (string oneParamString in paramStringList)
                                {
                                    ...
                                    {
                                        Utils.LogErrorAndExit(string.Format("声明的{0}参数下属的参数字符串{1}错误，参数名和配置值之间应用=分隔",
                                            AppValues.AUTO_NAME_CSV_CLASS_PARAM_STRING, oneParamString));
                                        return -1;
                                    }
                                    else
                                    {
                                        ...
                                        if (AppValues.AUTO_NAME_CSV_CLASS_PARAM_CLASS_NAME_PREFIX_PARAM_STRING.Equals(key,
                                        ...
                                        else if (AppValues.AUTO_NAME_CSV_CLASS_PARAM_CLASS_NAME_POSTFIX_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                        ...
                                        {
                                            Utils.LogErrorAndExit(string.Format("错误：声明的{0}参数下属的参数{1}非法", AppValues.AUTO_NAME_CSV_CLASS_PARAM_STRING,
                                                key));
                                            return -1;
                                        }
                                    }
                    ...
                    else if (param.StartsWith(AppValues.EXPORT_CSV_PARAM_STRING, StringComparison.CurrentCultureIgnoreCase))
                    {
                        ...
                        if (exportCsvParamString != null)
                        {
                            ...
                            else
                            {
                                ...
                                foreach (string oneParamString in paramStringList)
                                {
                                    ...
                                    {
                                        Utils.LogErrorAndExit(string.Format("声明的{0}参数下属的参数字符串{1}错误，参数名和配置值之间应用=分隔",
                                            AppValues.EXPORT_CSV_PARAM_PARAM_STRING, oneParamString));
                                        return -1;
                                    }
                                    else
                                    {
                                        ...
                                        )
                                        {
                                            ...
                                            {
                                                Utils.LogErrorAndExit(string.Format("错误：声明的{0}参数下属的参数{1}所配置的csv文件导出路径不存在",
                                                    AppValues.EXPORT_CSV_PARAM_PARAM_STRING, AppValues.EXPORT_CSV_PARAM_EXPORT_PATH_PARAM_STRING));
                                                return -1;
                                            }
                                            else
                                                AppValues.ExportCsvPath = Path.GetFullPath(value);
                                        }
                                        else if (AppValues.EXPORT_CSV_PARAM_EXTENSION_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            value = value.Trim();
                                            if (string.IsNullOrEmpty(value))
                                            {
                                                Utils.LogErrorAndExit(string.Format("错误：声明的{0}参数下属的参数{1}所配置的导出csv文件的扩展名不允许为空",
                                                    AppValues.EXPORT_CSV_PARAM_PARAM_STRING, AppValues.EXPORT_CSV_PARAM_EXTENSION_PARAM_STRING));
                                                return -1;
                                            }
                                            if (value.StartsWith("."))
                                        ...
                                        else if (AppValues.EXPORT_CSV_PARAM_SPLIT_STRING_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            value = value.Replace("\\|", "|");
                                            AppValues.ExportCsvSplitString = value;
                                        }
                                        else if (AppValues.EXPORT_CSV_PARAM_IS_EXPORT_COLUMN_NAME_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            value = value.Trim();
                                            ...
                                            {
                                                Utils.LogErrorAndExit(string.Format("错误：声明的{0}参数下属的参数{1}所配置的值错误，必须为true或false",
                                                    AppValues.EXPORT_CSV_PARAM_PARAM_STRING,
                                                    AppValues.EXPORT_CSV_PARAM_IS_EXPORT_COLUMN_NAME_PARAM_STRING));
                                                return -1;
                                            }
                                        }
                                        else if (AppValues.EXPORT_CSV_PARAM_IS_EXPORT_COLUMN_DATA_TYPE_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            value = value.Trim();
                                            if ("true".Equals(value, StringComparison.CurrentCultureIgnoreCase))
                                            ...
                                            else
                                            {
                                                Utils.LogErrorAndExit(string.Format("错误：声明的{0}参数下属的参数{1}所配置的值错误，必须为true或false",
                                                    AppValues.EXPORT_CSV_PARAM_PARAM_STRING,
                                                    AppValues.EXPORT_CSV_PARAM_IS_EXPORT_COLUMN_DATA_TYPE_PARAM_STRING));
                                                return -1;
                                            }
                                        }
                                        else
                                        {
                                            Utils.LogErrorAndExit(
                                                string.Format("错误：声明的{0}参数下属的参数{1}非法", AppValues.EXPORT_CSV_PARAM_PARAM_STRING, key));
                                            return -1;
                                        }
                                    }
                                }
                                ...
                                {
                                    Utils.LogErrorAndExit(string.Format("错误：声明要额外导出指定Excel文件为csv文件，就必须同时在{0}参数下声明用于配置csv文件导出路径的参数{1}",
                                        AppValues.EXPORT_CSV_PARAM_PARAM_STRING, AppValues.EXPORT_CSV_PARAM_EXPORT_PATH_PARAM_STRING));
                                    return -1;
                                }
                            }
                        ...
                        {
                            Utils.LogErrorAndExit(string.Format("错误：声明要额外导出指定Excel文件为csv文件，就必须同时声明用于配置csv文件导出参数的{0}",
                                AppValues.EXPORT_CSV_PARAM_PARAM_STRING));
                            return -1;
                        }
                        ...
                        {
                            Utils.LogErrorAndExit(string.Format("必须在英文小括号内声明要导出为csv文件的Excel表格名，若要全部导出，请配置为{0}参数",
                                AppValues.EXPORT_ALL_TO_EXTRA_FILE_PARAM_STRING));
                            return -1;
                        }

                        string exportCsvFileWithoutBracket =
                            param.Substring(paramLeftBracketIndex + 1, paramRightBracketIndex - paramLeftBracketIndex - 1).Trim();
                        if (exportCsvFileWithoutBracket.Equals(AppValues.EXPORT_ALL_TO_EXTRA_FILE_PARAM_STRING,
                            StringComparison.CurrentCultureIgnoreCase))
                    ...
                    else if (param.StartsWith(AppValues.EXPORT_CS_CLASS_PARAM_STRING, StringComparison.CurrentCultureIgnoreCase))
                    {
                        ...
                        if (exportCsClassParamString != null)
                        {
                            ...
                            if (leftBracketIndex == -1 || rightBracketIndex == -1 || leftBracketIndex > rightBracketIndex)
                            {
                                Utils.LogErrorAndExit(string.Format("错误：声明导出csv对应C#类文件的参数{0}后必须在英文小括号内声明各个具体参数",
                                    AppValues.EXPORT_CS_CLASS_PARAM_PARAM_STRING));
                                return -1;
                            }
                            else
                            {
                                string paramString =
                                    exportCsClassParamString.Substring(leftBracketIndex + 1, rightBracketIndex - leftBracketIndex - 1);
                                // 通过|分隔各个参数
                                string[] paramStringList = paramString.Split(new char[] {'|'}, StringSplitOptions.RemoveEmptyEntries);
                                // 解析各个具体参数
                                foreach (string oneParamString in paramStringList)
                                {
                                    ...
                                    {
                                        Utils.LogErrorAndExit(string.Format("声明的{0}参数下属的参数字符串{1}错误，参数名和配置值之间应用=分隔",
                                            AppValues.EXPORT_CS_CLASS_PARAM_PARAM_STRING, oneParamString));
                                        return -1;
                                    }
                                    else
                                    {
                                        string key = keyAndValue[0].Trim();
                                        string value = keyAndValue[1];
                                        if (AppValues.EXPORT_CS_CLASS_PARAM_EXPORT_PATH_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            // 检查导出路径是否存在
                                            if (!Directory.Exists(value))
                                            {
                                                Utils.LogErrorAndExit(string.Format("错误：声明的{0}参数下属的参数{1}所配置的导出csv对应C#类文件导出路径不存在",
                                                    AppValues.EXPORT_CS_CLASS_PARAM_PARAM_STRING,
                                                    AppValues.EXPORT_CS_CLASS_PARAM_EXPORT_PATH_PARAM_STRING));
                                                return -1;
                                            }
                                            else
                                                AppValues.ExportCsClassPath = Path.GetFullPath(value);
                                        }
                                        else if (AppValues.EXPORT_CS_CLASS_PARAM_NAMESPACE_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            value = value.Trim();
                                            if (string.IsNullOrEmpty(value))
                                                Utils.LogWarning(string.Format(
                                                    "警告：声明的{0}参数下属的参数{1}所配置的导出csv对应C#类文件中的命名空间为空，若不想设置命名空间，可以直接不配置此参数，而不是将参数值留空",
                                                    AppValues.EXPORT_CS_CLASS_PARAM_PARAM_STRING,
                                                    AppValues.EXPORT_CS_CLASS_PARAM_NAMESPACE_PARAM_STRING));
                                            else
                                                AppValues.ExportCsClassNamespace = value;
                                        }
                                        else if (AppValues.EXPORT_CS_CLASS_PARAM_USING_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            value = value.Trim();
                                            if (string.IsNullOrEmpty(value))
                                                Utils.LogWarning(string.Format(
                                                    "警告：声明的{0}参数下属的参数{1}所配置的导出csv对应C#类文件中的引用类库为空，若不想设置引用类库，可以直接不配置此参数，而不是将参数值留空",
                                                    AppValues.EXPORT_CS_CLASS_PARAM_PARAM_STRING,
                                                    AppValues.EXPORT_CS_CLASS_PARAM_USING_PARAM_STRING));
                                            else
                                            {
                                                // 通过英文逗号分隔各个引用类库
                                                ...
                                                for (int index = 0; index < usingList.Length; ++index)
                                                {
                                                    ...
                                                    if (exportCsClassUsing.Count > 0)
                                                        ...
                                                        Utils.LogWarning(string.Format(
                                                            "警告：声明的{0}参数下属的参数{1}所配置的导出csv对应C#类文件中的引用类库均为空，若不想设置引用类库，可以直接不配置此参数，而不是将参数值留空",
                                                            AppValues.EXPORT_CS_CLASS_PARAM_PARAM_STRING,
                                                            AppValues.EXPORT_CS_CLASS_PARAM_USING_PARAM_STRING));
                                                }
                                            }
                                        ...
                                        {
                                            Utils.LogErrorAndExit(string.Format("错误：声明的{0}参数下属的参数{1}非法", AppValues.EXPORT_CS_CLASS_PARAM_PARAM_STRING,
                                                key));
                                            return -1;
                                        }
                                    }
                                ...
                                {
                                    Utils.LogErrorAndExit(string.Format("错误：声明要额外导出csv对应C#类文件，就必须同时在{0}参数下声明用于配置导出路径的参数{1}",
                                        AppValues.EXPORT_CS_CLASS_PARAM_PARAM_STRING, AppValues.EXPORT_CS_CLASS_PARAM_EXPORT_PATH_PARAM_STRING));
                                    return -1;
                                }
                            }
                        }
                        ...
                        {
                            Utils.LogErrorAndExit(string.Format("错误：声明要额外导出指定Excel文件为csv对应C#类文件，就必须同时声明用于配置C#类文件导出参数的{0}",
                                AppValues.EXPORT_CS_CLASS_PARAM_PARAM_STRING));
                            return -1;
                        }
                        ...
                        if (paramLeftBracketIndex == -1 || paramRightBracketIndex == -1 || paramLeftBracketIndex > paramRightBracketIndex)
                        {
                            Utils.LogErrorAndExit(string.Format("必须在英文小括号内声明要导出为csv对应C#类文件的Excel表格名，若要全部导出，请配置为{0}参数",
                                AppValues.EXPORT_ALL_TO_EXTRA_FILE_PARAM_STRING));
                            return -1;
                        }

                        string exportCsClassFileWithoutBracket =
                            param.Substring(paramLeftBracketIndex + 1, paramRightBracketIndex - paramLeftBracketIndex - 1).Trim();
                        if (exportCsClassFileWithoutBracket.Equals(AppValues.EXPORT_ALL_TO_EXTRA_FILE_PARAM_STRING,
                            StringComparison.CurrentCultureIgnoreCase))
                        ...
                        else
                        {
                            ...
                            if (errorString != null)
                            {
                                Utils.LogErrorAndExit(string.Format("错误：声明额外导出为csv对应C#类文件的参数{0}后{1}", AppValues.EXPORT_CS_CLASS_PARAM_STRING,
                                    errorString));
                                return -1;
                            }
                            else
                            {
                                ...
                                foreach (string fileName in fileNames)
                                {
                                    if (!existExcelFileNames.Contains(fileName))
                                    {
                                        Utils.LogErrorAndExit(string.Format("要求额外导出为csv对应C#类文件的Excel表（{0}）不存在，请检查后重试并注意区分大小写",
                                            Utils.CombinePath(AppValues.ExportCsClassPath, string.Concat(fileName, ".xlsx"))));
                                        return -1;
                                    }
                                    else
                    ...
                    else if (param.StartsWith(AppValues.EXPORT_JAVA_CLASS_PARAM_STRING, StringComparison.CurrentCultureIgnoreCase))
                    {
                        ...
                        if (exportJavaClassParamString != null)
                        {
                            ...
                            if (leftBracketIndex == -1 || rightBracketIndex == -1 || leftBracketIndex > rightBracketIndex)
                            {
                                Utils.LogErrorAndExit(string.Format("错误：声明导出csv对应Java类文件的参数{0}后必须在英文小括号内声明各个具体参数",
                                    AppValues.EXPORT_JAVA_CLASS_PARAM_PARAM_STRING));
                                return -1;
                            }
                            else
                            {
                                string paramString =
                                    exportJavaClassParamString.Substring(leftBracketIndex + 1, rightBracketIndex - leftBracketIndex - 1);
                                // 通过|分隔各个参数
                                string[] paramStringList = paramString.Split(new char[] {'|'}, StringSplitOptions.RemoveEmptyEntries);
                                // 解析各个具体参数
                                foreach (string oneParamString in paramStringList)
                                {
                                    ...
                                    {
                                        Utils.LogErrorAndExit(string.Format("声明的{0}参数下属的参数字符串{1}错误，参数名和配置值之间应用=分隔",
                                            AppValues.EXPORT_JAVA_CLASS_PARAM_PARAM_STRING, oneParamString));
                                        return -1;
                                    }
                                    else
                                    {
                                        string key = keyAndValue[0].Trim();
                                        string value = keyAndValue[1];
                                        if (AppValues.EXPORT_JAVA_CLASS_PARAM_EXPORT_PATH_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            // 检查导出路径是否存在
                                            if (!Directory.Exists(value))
                                            {
                                                Utils.LogErrorAndExit(string.Format("错误：声明的{0}参数下属的参数{1}所配置的导出csv对应Java类文件导出路径不存在",
                                                    AppValues.EXPORT_JAVA_CLASS_PARAM_PARAM_STRING,
                                                    AppValues.EXPORT_JAVA_CLASS_PARAM_EXPORT_PATH_PARAM_STRING));
                                                return -1;
                                            }
                                            else
                                                AppValues.ExportJavaClassPath = Path.GetFullPath(value);
                                        }
                                        else if (AppValues.EXPORT_JAVA_CLASS_PARAM_PACKAGE_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            value = value.Trim();
                                            if (!string.IsNullOrEmpty(value))
                                        ...
                                        else if (AppValues.EXPORT_JAVA_CLASS_PARAM_IMPORT_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            value = value.Trim();
                                            if (string.IsNullOrEmpty(value))
                                                Utils.LogWarning(string.Format(
                                                    "警告：声明的{0}参数下属的参数{1}所配置的导出csv对应Java类文件中的引用类库为空，若不想设置引用类库，可以直接不配置此参数，而不是将参数值留空",
                                                    AppValues.EXPORT_JAVA_CLASS_PARAM_PARAM_STRING,
                                                    AppValues.EXPORT_JAVA_CLASS_PARAM_IMPORT_PARAM_STRING));
                                            else
                                            {
                                                // 通过英文逗号分隔各个引用类库
                                                ...
                                                for (int index = 0; index < importList.Length; ++index)
                                                {
                                                    ...
                                                    if (exportJavaClassPackage.Count > 0)
                                                        ...
                                                        Utils.LogWarning(string.Format(
                                                            "警告：声明的{0}参数下属的参数{1}所配置的导出csv对应Java类文件中的引用类库均为空，若不想设置引用类库，可以直接不配置此参数，而不是将参数值留空",
                                                            AppValues.EXPORT_JAVA_CLASS_PARAM_PARAM_STRING,
                                                            AppValues.EXPORT_JAVA_CLASS_PARAM_PACKAGE_PARAM_STRING));
                                                }
                                            }
                                        }
                                        else if (AppValues.EXPORT_JAVA_CLASS_PARAM_IS_USE_DATE_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            value = value.Trim();
                                            ...
                                            else
                                            {
                                                Utils.LogErrorAndExit(string.Format(
                                                    "警告：声明的{0}参数下属的参数{1}所配置的导出csv对应Java类文件中的时间型是否转为Date而不是Calendar型的选项值配置错误，必须配置为true或false",
                                                    AppValues.EXPORT_JAVA_CLASS_PARAM_PARAM_STRING,
                                                    AppValues.EXPORT_JAVA_CLASS_PARAM_IS_USE_DATE_PARAM_STRING));
                                                return -1;
                                            }
                                        }
                                        else if (AppValues.EXPORT_JAVA_CLASS_PARAM_IS_GENERATE_CONSTRUCTOR_WITHOUT_FIELDS_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            value = value.Trim();
                                            if (value.Equals("true", StringComparison.CurrentCultureIgnoreCase))
                                            ...
                                            {
                                                Utils.LogErrorAndExit(string.Format(
                                                    "警告：声明的{0}参数下属的参数{1}所配置的导出csv对应Java类文件是否生成无参构造函数的选项值配置错误，必须配置为true或false",
                                                    AppValues.EXPORT_JAVA_CLASS_PARAM_PARAM_STRING,
                                                    AppValues.EXPORT_JAVA_CLASS_PARAM_IS_GENERATE_CONSTRUCTOR_WITHOUT_FIELDS_PARAM_STRING));
                                                return -1;
                                            }
                                        }
                                        else if (AppValues.EXPORT_JAVA_CLASS_PARAM_IS_GENERATE_CONSTRUCTOR_WITH_ALL_FIELDS_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            value = value.Trim();
                                            if (value.Equals("true", StringComparison.CurrentCultureIgnoreCase))
                                            ...
                                            {
                                                Utils.LogErrorAndExit(string.Format(
                                                    "警告：声明的{0}参数下属的参数{1}所配置的导出csv对应Java类文件是否生成含所有参数的构造函数的选项值配置错误，必须配置为true或false",
                                                    AppValues.EXPORT_JAVA_CLASS_PARAM_PARAM_STRING,
                                                    AppValues.EXPORT_JAVA_CLASS_PARAM_IS_GENERATE_CONSTRUCTOR_WITH_ALL_FIELDS_PARAM_STRING));
                                                return -1;
                                            }
                                        }
                                        else
                                        {
                                            Utils.LogErrorAndExit(string.Format("错误：声明的{0}参数下属的参数{1}非法",
                                                AppValues.EXPORT_JAVA_CLASS_PARAM_PARAM_STRING, key));
                                            return -1;
                                        }
                                    }
                                }
                                ...
                                {
                                    Utils.LogErrorAndExit(string.Format("错误：声明要额外导出csv对应Java类文件，就必须同时在{0}参数下声明用于配置导出路径的参数{1}",
                                        AppValues.EXPORT_JAVA_CLASS_PARAM_PARAM_STRING, AppValues.EXPORT_JAVA_CLASS_PARAM_EXPORT_PATH_PARAM_STRING));
                                    return -1;
                                }
                                // 要求必须含有package参数
                                if (AppValues.ExportJavaClassPackage == null)
                                {
                                    Utils.LogErrorAndExit(string.Format("错误：声明要额外导出csv对应Java类文件，就必须同时在{0}参数下声明用于配置包名的参数{1}",
                                        AppValues.EXPORT_JAVA_CLASS_PARAM_PARAM_STRING, AppValues.EXPORT_JAVA_CLASS_PARAM_PACKAGE_PARAM_STRING));
                                    return -1;
                                }
                            }
                        }
                        ...
                        {
                            Utils.LogErrorAndExit(string.Format("错误：声明要额外导出指定Excel文件为csv对应Java类文件，就必须同时声明用于配置Java类文件导出参数的{0}",
                                AppValues.EXPORT_JAVA_CLASS_PARAM_PARAM_STRING));
                            return -1;
                        }
                        ...
                        if (paramLeftBracketIndex == -1 || paramRightBracketIndex == -1 || paramLeftBracketIndex > paramRightBracketIndex)
                        {
                            Utils.LogErrorAndExit(string.Format("必须在英文小括号内声明要导出为csv对应Java类文件的Excel表格名，若要全部导出，请配置为{0}参数",
                                AppValues.EXPORT_ALL_TO_EXTRA_FILE_PARAM_STRING));
                            return -1;
                        }

                        string exportJavaClassFileWithoutBracket =
                            param.Substring(paramLeftBracketIndex + 1, paramRightBracketIndex - paramLeftBracketIndex - 1).Trim();
                        if (exportJavaClassFileWithoutBracket.Equals(AppValues.EXPORT_ALL_TO_EXTRA_FILE_PARAM_STRING,
                            StringComparison.CurrentCultureIgnoreCase))
                        ...
                        else
                        {
                            ...
                            if (errorString != null)
                            {
                                Utils.LogErrorAndExit(string.Format("错误：声明额外导出为csv对应Java类文件的参数{0}后{1}", AppValues.EXPORT_JAVA_CLASS_PARAM_STRING,
                                    errorString));
                                return -1;
                            }
                            else
                            {
                                ...
                                foreach (string fileName in fileNames)
                                {
                                    if (!existExcelFileNames.Contains(fileName))
                                    {
                                        Utils.LogErrorAndExit(string.Format("要求额外导出为csv对应Java类文件的Excel表（{0}）不存在，请检查后重试并注意区分大小写",
                                            Utils.CombinePath(AppValues.ExportJavaClassPath, string.Concat(fileName, ".xlsx"))));
                                        return -1;
                                    }
                                    else
                    ...
                    else if (param.StartsWith(AppValues.EXPORT_JSON_PARAM_STRING, StringComparison.CurrentCultureIgnoreCase))
                    {
                        ...
                        if (exportJsonParamString != null)
                        {
                            ...
                            {
                                Utils.LogErrorAndExit(
                                    string.Format("错误：声明导出json文件的参数{0}后必须在英文小括号内声明各个具体参数", AppValues.EXPORT_JSON_PARAM_PARAM_STRING));
                                return -1;
                            }
                            else
                            {
                                ...
                                foreach (string oneParamString in paramStringList)
                                {
                                    ...
                                    {
                                        Utils.LogErrorAndExit(string.Format("声明的{0}参数下属的参数字符串{1}错误，参数名和配置值之间应用=分隔",
                                            AppValues.EXPORT_JSON_PARAM_PARAM_STRING, oneParamString));
                                        return -1;
                                    }
                                    else
                                    {
                                        string key = keyAndValue[0].Trim();
                                        string value = keyAndValue[1];
                                        if (AppValues.EXPORT_JSON_PARAM_EXPORT_PATH_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            // 检查导出路径是否存在
                                            if (!Directory.Exists(value))
                                            {
                                                Utils.LogErrorAndExit(string.Format("错误：声明的{0}参数下属的参数{1}所配置的json文件导出路径不存在",
                                                    AppValues.EXPORT_JSON_PARAM_PARAM_STRING, AppValues.EXPORT_JSON_PARAM_EXPORT_PATH_PARAM_STRING));
                                                return -1;
                                            }
                                            else
                                                AppValues.ExportJsonPath = Path.GetFullPath(value);
                                        }
                                        else if (AppValues.EXPORT_JSON_PARAM_EXTENSION_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            value = value.Trim();
                                            if (string.IsNullOrEmpty(value))
                                            {
                                                Utils.LogErrorAndExit(string.Format("错误：声明的{0}参数下属的参数{1}所配置的导出json文件的扩展名不允许为空",
                                                    AppValues.EXPORT_JSON_PARAM_PARAM_STRING, AppValues.EXPORT_JSON_PARAM_EXTENSION_PARAM_STRING));
                                                return -1;
                                            }
                                            if (value.StartsWith("."))
                                                value = value.Substring(1);
                                        ...
                                        else if (AppValues.EXPORT_JSON_PARAM_IS_FORMAT_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            value = value.Trim();
                                            if ("true".Equals(value, StringComparison.CurrentCultureIgnoreCase))
                                            ...
                                            {
                                                Utils.LogErrorAndExit(string.Format("错误：声明的{0}参数下属的参数{1}所配置的值错误，必须为true或false",
                                                    AppValues.EXPORT_JSON_PARAM_PARAM_STRING, AppValues.EXPORT_JSON_PARAM_IS_FORMAT_PARAM_STRING));
                                                return -1;
                                            }
                                        }
                                        else if (AppValues.EXPORT_JSON_PARAM_IS_EXPORT_JSON_ARRAY_FORMAT_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            value = value.Trim();
                                            if ("true".Equals(value, StringComparison.CurrentCultureIgnoreCase))
                                            ...
                                            {
                                                Utils.LogErrorAndExit(string.Format("错误：声明的{0}参数下属的参数{1}所配置的值错误，必须为true或false",
                                                    AppValues.EXPORT_JSON_PARAM_PARAM_STRING,
                                                    AppValues.EXPORT_JSON_PARAM_IS_EXPORT_JSON_ARRAY_FORMAT_PARAM_STRING));
                                                return -1;
                                            }
                                        }
                                        else if (AppValues.EXPORT_JSON_PARAM_IS_MAP_INCLUDE_KEY_COLUMN_VALUE_PARAM_STRING.Equals(key,
                                            StringComparison.CurrentCultureIgnoreCase))
                                        {
                                            value = value.Trim();
                                            if ("true".Equals(value, StringComparison.CurrentCultureIgnoreCase))
                                            ...
                                            else
                                            {
                                                Utils.LogErrorAndExit(string.Format("错误：声明的{0}参数下属的参数{1}所配置的值错误，必须为true或false",
                                                    AppValues.EXPORT_JSON_PARAM_PARAM_STRING,
                                                    AppValues.EXPORT_JSON_PARAM_IS_MAP_INCLUDE_KEY_COLUMN_VALUE_PARAM_STRING));
                                                return -1;
                                            }
                                        }
                                        else
                                        {
                                            Utils.LogErrorAndExit(string.Format("错误：声明的{0}参数下属的参数{1}非法", AppValues.EXPORT_JSON_PARAM_PARAM_STRING,
                                                key));
                                            return -1;
                                        }
                                    }
                                ...
                                {
                                    Utils.LogErrorAndExit(string.Format("错误：声明要额外导出指定Excel文件为json文件，就必须同时在{0}参数下声明用于配置json文件导出路径的参数{1}",
                                        AppValues.EXPORT_JSON_PARAM_PARAM_STRING, AppValues.EXPORT_JSON_PARAM_EXPORT_PATH_PARAM_STRING));
                                    return -1;
                                }
                            }
                        ...
                        {
                            Utils.LogErrorAndExit(string.Format("错误：声明要额外导出指定Excel文件为json文件，就必须同时声明用于配置json文件导出参数的{0}",
                                AppValues.EXPORT_JSON_PARAM_PARAM_STRING));
                            return -1;
                        }
                        ...
                        if (paramLeftBracketIndex == -1 || paramRightBracketIndex == -1 || paramLeftBracketIndex > paramRightBracketIndex)
                        {
                            Utils.LogErrorAndExit(string.Format("必须在英文小括号内声明要导出为json文件的Excel表格名，若要全部导出，请配置为{0}参数",
                                AppValues.EXPORT_ALL_TO_EXTRA_FILE_PARAM_STRING));
                            return -1;
                        }

                        string exportJsonFileWithoutBracket =
                            param.Substring(paramLeftBracketIndex + 1, paramRightBracketIndex - paramLeftBracketIndex - 1).Trim();
                        if (exportJsonFileWithoutBracket.Equals(AppValues.EXPORT_ALL_TO_EXTRA_FILE_PARAM_STRING,
                            StringComparison.CurrentCultureIgnoreCase))
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                foreach (string fileName in fileNames)
                                {
                                    if (!existExcelFileNames.Contains(fileName))
                                    {
                                        Utils.LogErrorAndExit(string.Format("要求额外导出为json文件的Excel表（{0}）不存在，请检查后重试并注意区分大小写",
                                            Utils.CombinePath(AppValues.ExportJsonPath, string.Concat(fileName, ".xlsx"))));
                                        return -1;
                                    }
                                    else
                ...
                if (isExportPart == true)
                {
                    ...
                    {
                        Utils.LogErrorAndExit(string.Format("错误：以下表格既声明要进行导出，又声明要忽略导出：{0}，请修正配置后重试",
                            Utils.CombineString(errorConfigTableNames, ",")));
                        return -1;
                    }
                }
                ...
                if (warnExportCsvTableNames.Count > 0)
                {
                    Utils.LogWarning(string.Format("警告：以下Excel表声明为要额外导出为csv文件，但在{0}参数中未声明本次要对其进行导出，本工具将不对这些表格执行导出csv文件的操作\n{1}",
                        AppValues.PART_EXPORT_PARAM_STRING, Utils.CombineString(warnExportCsvTableNames, ", ")));
                ...
                if (warnExportJsonTableNames.Count > 0)
                {
                    Utils.LogWarning(string.Format("警告：以下Excel表声明为要额外导出为json文件，但在{0}参数中未声明本次要对其进行导出，本工具将不对这些表格执行导出json文件的操作\n{1}",
                        AppValues.PART_EXPORT_PARAM_STRING, Utils.CombineString(warnExportJsonTableNames, ", ")));
                ...
                if (AppValues.ConfigData.ContainsKey(AppValues.APP_CONFIG_KEY_DEFAULT_DATE_INPUT_FORMAT))
                {
                    ...
                    if (TableCheckHelper.CheckDateInputDefine(AppValues.DefaultDateInputFormat, out tempErrorString) == false)
                        errorStringBuilder.AppendFormat(ERROR_STRING_FORMAT, AppValues.APP_CONFIG_KEY_DEFAULT_DATE_INPUT_FORMAT,
                            AppValues.DefaultDateInputFormat, tempErrorString);
                }
                ...
                if (AppValues.ConfigData.ContainsKey(AppValues.APP_CONFIG_KEY_DEFAULT_DATE_TO_LUA_FORMAT))
                {
                    ...
                    if (TableCheckHelper.CheckDateToLuaDefine(AppValues.DefaultDateToLuaFormat, out tempErrorString) == false)
                        errorStringBuilder.AppendFormat(ERROR_STRING_FORMAT, AppValues.APP_CONFIG_KEY_DEFAULT_DATE_TO_LUA_FORMAT,
                            AppValues.DefaultDateToLuaFormat, tempErrorString);
                }
                ...
                if (AppValues.ConfigData.ContainsKey(AppValues.APP_CONFIG_KEY_DEFAULT_DATE_TO_DATABASE_FORMAT))
                {
                    ...
                    if (TableCheckHelper.CheckDateToDatabaseDefine(AppValues.DefaultDateToDatabaseFormat, out tempErrorString) == false)
                        errorStringBuilder.AppendFormat(ERROR_STRING_FORMAT, AppValues.APP_CONFIG_KEY_DEFAULT_DATE_TO_DATABASE_FORMAT,
                            AppValues.DefaultDateToDatabaseFormat, tempErrorString);
                }
                ...
                if (AppValues.ConfigData.ContainsKey(AppValues.APP_CONFIG_KEY_DEFAULT_TIME_INPUT_FORMAT))
                {
                    ...
                    if (TableCheckHelper.CheckTimeDefine(AppValues.DefaultTimeInputFormat, out tempErrorString) == false)
                        errorStringBuilder.AppendFormat(ERROR_STRING_FORMAT, AppValues.APP_CONFIG_KEY_DEFAULT_TIME_INPUT_FORMAT,
                            AppValues.DefaultTimeInputFormat, tempErrorString);
                }
                ...
                if (AppValues.ConfigData.ContainsKey(AppValues.APP_CONFIG_KEY_DEFAULT_TIME_TO_LUA_FORMAT))
                {
                    ...
                    if (TableCheckHelper.CheckTimeDefine(AppValues.DefaultTimeToLuaFormat, out tempErrorString) == false)
                        errorStringBuilder.AppendFormat(ERROR_STRING_FORMAT, AppValues.APP_CONFIG_KEY_DEFAULT_TIME_TO_LUA_FORMAT,
                            AppValues.DefaultTimeToLuaFormat, tempErrorString);
                }
                ...
                if (AppValues.ConfigData.ContainsKey(AppValues.APP_CONFIG_KEY_DEFAULT_TIME_TO_DATABASE_FORMAT))
                {
                    ...
                    if (TableCheckHelper.CheckTimeDefine(AppValues.DefaultTimeToDatabaseFormat, out tempErrorString) == false)
                        errorStringBuilder.AppendFormat(ERROR_STRING_FORMAT, AppValues.APP_CONFIG_KEY_DEFAULT_TIME_TO_DATABASE_FORMAT,
                            AppValues.DefaultTimeToDatabaseFormat, tempErrorString);
                }
                ...
                Utils.Log(string.Format("开始解析Excel所在目录下的所有Excel文件（{0}）：",
                    AppValues.IsExportIncludeSubfolder == true ? "包含子目录中的Excel文件" : "不包含子目录中的Excel文件"));
                Stopwatch stopwatch = new Stopwatch();
                // 注意：不管Excel表格本次是否需要进行导出，都要进行解析，因为其他表格中可能含有对那些表格的引用检查
                string[] excelFilePath = Utils.GetSupportedExcelFilePaths(AppValues.ExcelFolderPath,
                    AppValues.IsExportIncludeSubfolder == true ? SearchOption.AllDirectories : SearchOption.TopDirectoryOnly).ToArray();
                        ...
                        else
                        {
                            ...
                            if (ds.Tables[AppValues.EXCEL_CONFIG_SHEET_NAME] != null)
                            {
                                tableInfo.TableConfig =
                                    TableAnalyzeHelper.GetTableConfig(ds.Tables[AppValues.EXCEL_CONFIG_SHEET_NAME], out errorString);
                                if (!string.IsNullOrEmpty(errorString))
                                {
                            ...
                            if (tableInfo.TableConfig != null && tableInfo.TableConfig.ContainsKey(AppValues.CONFIG_NAME_EXPORT))
                            {
                                ...
                                if (inputParams.Contains(AppValues.CONFIG_PARAM_NOT_EXPORT_ORIGINAL_TABLE))
                                {
                                    ...
                                    if (inputParams.Count == 1)
                                        Utils.LogWarning(string.Format("警告：你设置了不对表格\"{0}\"按默认方式进行导出，而又没有指定任何其他自定义导出规则，本工具对此表格不进行任何导出，请确认是否真要如此",
                                            tableInfo.TableName));
                                    else

    ...
    public class TableExportToMySQLHelper
    {
        ...
        public static bool ConnectToDatabase(out string errorString)
        {
            if (AppValues.ConfigData.ContainsKey(AppValues.APP_CONFIG_KEY_MYSQL_CONNECT_STRING))
            {
                ...
                if (_schemaName == null)
                {
                    errorString = string.Format("MySQL数据库连接字符串（\"{0}\"）中不包含Schema名的声明，请在{1}中任选一个参数名进行声明", connectString,
                        Utils.CombineString(_DEFINE_SCHEMA_NAME_PARAM, ","));
                    return false;
                }
        ...
        public static bool ExportTableToDatabase(TableInfo tableInfo, out string errorString)
        {
            ...
            if (tableInfo.TableConfig != null && tableInfo.TableConfig.ContainsKey(AppValues.CONFIG_NAME_EXPORT_DATABASE_TABLE_NAME))
            {
                ...
                string comment =
                    tableInfo.TableConfig != null && tableInfo.TableConfig.ContainsKey(AppValues.CONFIG_NAME_EXPORT_DATABASE_TABLE_COMMENT) &&
                    tableInfo.TableConfig[AppValues.CONFIG_NAME_EXPORT_DATABASE_TABLE_COMMENT].Count > 0
                        ? tableInfo.TableConfig[AppValues.CONFIG_NAME_EXPORT_DATABASE_TABLE_COMMENT][0]
                        : string.Empty;
        ...
        private static bool _InsertData(string tableName, TableInfo tableInfo, out string errorString)
        {
            ...
            bool isWriteNullForEmptyString = tableInfo.TableConfig != null &&
                                             tableInfo.TableConfig.ContainsKey(AppValues.CONFIG_NAME_EXPORT_DATABASE_WRITE_NULL_FOR_EMPTY_STRING) &&
                                             tableInfo.TableConfig[AppValues.CONFIG_NAME_EXPORT_DATABASE_WRITE_NULL_FOR_EMPTY_STRING].Count > 0 &&
                                             "true".Equals(
                                                 tableInfo.TableConfig[AppValues.CONFIG_NAME_EXPORT_DATABASE_WRITE_NULL_FOR_EMPTY_STRING][0],
            ...
            if (count > 0)
            {
                for (int i = 0; i < count; ++i)
                {
                    ...
                    foreach (FieldInfo fieldInfo in allDatabaseFieldInfo)
                    {
                        ...
                        {
                            string toDatabaseFormatDefine =
                                fieldInfo.ExtraParam[AppValues.TABLE_INFO_EXTRA_PARAM_KEY_DATE_TO_DATABASE_FORMAT].ToString();
                            ...
                            if (toDatabaseFormatType == DateFormatType.FormatString)
                            {
                                ...
                                if (fieldInfo.DatabaseFieldType.Equals("date", StringComparison.CurrentCultureIgnoreCase))
                                    values.Add(string.Format("'{0}'",
                                        ((DateTime) (fieldInfo.Data[i])).ToString(AppValues.APP_DEFAULT_ONLY_DATE_FORMAT)));
                                ...
                                // date型导出到MySQL中的其他数据类型字段如varchar，采用声明的指定格式
                                    ...
                                    values.Add(string.Format("'{0}'",
                                        ((DateTime) (fieldInfo.Data[i])).ToString(fieldInfo
                                            .ExtraParam[AppValues.TABLE_INFO_EXTRA_PARAM_KEY_DATE_TO_DATABASE_FORMAT].ToString())));
                            }
                        ...
                        {
                            string toDatabaseFormatDefine =
                                fieldInfo.ExtraParam[AppValues.TABLE_INFO_EXTRA_PARAM_KEY_TIME_TO_DATABASE_FORMAT].ToString();
                            ...
                            if (toDatabaseFormatType == TimeFormatType.FormatString)
                            {
                                if (fieldInfo.DatabaseFieldType.StartsWith("time", StringComparison.CurrentCultureIgnoreCase))
                                    ...
                                    values.Add(string.Format("'{0}'",
                                        ((DateTime) (fieldInfo.Data[i])).ToString(fieldInfo
                                            .ExtraParam[AppValues.TABLE_INFO_EXTRA_PARAM_KEY_TIME_TO_DATABASE_FORMAT].ToString())));
                            }
                        ...
                        else if (fieldInfo.DataType == DataType.Bool)
                        {
                            ...
                            if (fieldInfo.DatabaseFieldType.Equals("bit", StringComparison.CurrentCultureIgnoreCase) ||
                                fieldInfo.DatabaseFieldType.StartsWith("bit(", StringComparison.CurrentCultureIgnoreCase))
                            {
                                if (inputData == true)
                                    values.Add("1");
                                else
                ...
                string insertSqlString = string.Format(_INSERT_DATA_SQL, _CombineDatabaseTableFullName(tableName), fieldNameDefineString,
                    valueDefineString);
        ...
        private static bool _CreateTable(string tableName, TableInfo tableInfo, string comment, out string errorString)
        {
            ...
            foreach (FieldInfo fieldInfo in GetAllDatabaseFieldInfo(tableInfo))
            {
                ...
                if (fieldInfo.DataType == DataType.Date)
                {
                    ...
                    if (fieldInfo.DatabaseFieldType.StartsWith("time", StringComparison.CurrentCultureIgnoreCase))
                    {
                        errorString = string.Format("date型字段\"{0}\"（列号：{1}）声明导出到MySQL中的数据类型错误，不允许为time型，如果仅需要时分秒部分，请在Excel中将该字段在本工具中的数据类型改为time型",
                    ...
                    {
                        if (fieldInfo.DatabaseFieldType.StartsWith("datetime", StringComparison.CurrentCultureIgnoreCase) ||
                            fieldInfo.DatabaseFieldType.Equals("date", StringComparison.CurrentCultureIgnoreCase))
                        {
                            errorString = string.Format(
                                "date型字段\"{0}\"（列号：{1}）声明导出到MySQL中的形式为距1970年的时间（{2}），但所填写的导出到MySQL中的格式为时间型的{3}，请声明为MySQL中的数值型", fieldInfo.FieldName,
                                Utils.GetExcelColumnName(fieldInfo.ColumnSeq + 1), toDatabaseFormatDefine, fieldInfo.DatabaseFieldType);
                            return false;
                        }
                    }
                }
                else if (fieldInfo.DataType == DataType.Time)
                {
                    ...
                    {
                        errorString = string.Format(
                            "time型字段\"{0}\"（列号：{1}）声明导出到MySQL中的数据类型错误，不允许为datetime或date型，如果需要年月日部分，请在Excel中将该字段在本工具中的数据类型改为date型",
                            fieldInfo.FieldName, Utils.GetExcelColumnName(fieldInfo.ColumnSeq + 1));
                        return false;
                    }

                    if (toDatabaseFormatType == TimeFormatType.ReferenceTimeSec &&
                        fieldInfo.DatabaseFieldType.StartsWith("time", StringComparison.CurrentCultureIgnoreCase))
                    {
                        errorString = string.Format("time型字段\"{0}\"（列号：{1}）声明导出到MySQL中的形式为距0点的秒数（#sec），但所填写的导出到MySQL中的格式为时间型的time，请声明为MySQL中的数值型",
                            fieldInfo.FieldName, Utils.GetExcelColumnName(fieldInfo.ColumnSeq + 1));
                        return false;
                    }
                }

                fieldDefineStringBuilder.AppendFormat("`{0}` {1} COMMENT '{2}',", fieldInfo.DatabaseFieldName, fieldInfo.DatabaseFieldType,
                    fieldInfo.Desc);
            }

            string createTableExtraParam = AppValues.ConfigData.ContainsKey(AppValues.APP_CONFIG_KEY_CREATE_DATABASE_TABLE_EXTRA_PARAM)
                ? AppValues.ConfigData[AppValues.APP_CONFIG_KEY_CREATE_DATABASE_TABLE_EXTRA_PARAM]
                : string.Empty;
            string createTableSql = string.Format(_CREATE_TABLE_SQL, _CombineDatabaseTableFullName(tableName), fieldDefineStringBuilder.ToString(),
                tableInfo.GetKeyColumnFieldInfo().DatabaseFieldName, comment, createTableExtraParam);

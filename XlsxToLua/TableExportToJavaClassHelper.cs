...
namespace XlsxToLua
{
    public class TableExportToJavaClassHelper
    {
        ...
        public static bool ExportTableToJavaClass(TableInfo tableInfo, out string errorString)
        {
            ...
            if (isNeedDateTypeImport == true && AppValues.ExportJavaClassIsUseDate == true &&
                (AppValues.ExportJavaClassImport == null || !AppValues.ExportJavaClassImport.Contains(_DATE_DATA_TYPE_TO_DATE_IMPORT_STRING)))
            {
                stringBuilder.AppendLine(string.Concat("import ", _DATE_DATA_TYPE_TO_DATE_IMPORT_STRING, ";"));
                isAddImport = true;
            }

            if (isNeedDateTypeImport == true && AppValues.ExportJavaClassIsUseDate == false &&
                (AppValues.ExportJavaClassImport == null || !AppValues.ExportJavaClassImport.Contains(_DATE_DATA_TYPE_TO_CALENDAR_IMPORT_STRING)))
            {
            ...
            if (tableInfo.TableConfig != null && tableInfo.TableConfig.ContainsKey(AppValues.CONFIG_NAME_EXPORT_CSV_CLASS_NAME) &&
                tableInfo.TableConfig[AppValues.CONFIG_NAME_EXPORT_CSV_CLASS_NAME].Count > 0 &&
                !string.IsNullOrEmpty(tableInfo.TableConfig[AppValues.CONFIG_NAME_EXPORT_CSV_CLASS_NAME][0].Trim()))
            ...
            foreach (FieldInfo fieldInfo in allFieldInfo)
            {
                ...
                stringBuilder.AppendLine(string.Concat("set", fieldInfo.FieldName.Substring(0, 1).ToUpper(), fieldInfo.FieldName.Substring(1), "(",
                    _GetJavaClassFieldDefine(fieldInfo), " ", fieldInfo.FieldName, ") {"));
                ++level;
                stringBuilder.Append(_GetJavaClassIndentation(level));
                ...
                case DataType.TableString:
                {
                    if (fieldInfo.TableStringFormatDefine.KeyDefine.KeyType == TableStringKeyType.Seq)
                    {
                        ...
                        switch (fieldInfo.TableStringFormatDefine.ValueDefine.ValueType)
                        {
                            ...
                            {
                                string valueDataTypeString =
                                    _GetJavaClassTableStringDataType(fieldInfo.TableStringFormatDefine.ValueDefine.DataInIndexDefine.DataType);
                                if (valueDataTypeString == null)
                                    Utils.LogErrorAndExit(
                                        "用_GetJavaClassFieldDefine函数导出csv对应Java类文件中tableString型字段的seq型key的dataInIndex型value的数据类型非法");
                                else
                    ...
                    else if (fieldInfo.TableStringFormatDefine.KeyDefine.KeyType == TableStringKeyType.DataInIndex)
                    {
                        ...
                        // key
                        string keyDataTypeString =
                            _GetJavaClassTableStringDataType(fieldInfo.TableStringFormatDefine.KeyDefine.DataInIndexDefine.DataType);
                        ...
                        switch (fieldInfo.TableStringFormatDefine.ValueDefine.ValueType)
                        {
                            ...
                            case TableStringValueType.DataInIndex:
                            {
                                ...
                                if (valueDataTypeString == null)
                                    Utils.LogErrorAndExit(
                                        "用_GetJavaClassFieldDefine函数导出csv对应Java类文件中tableString型字段的dataInIndex型key的dataInIndex型value的数据类型非法");
                                else
                                    stringBuilder.Append(valueDataTypeString);

...
namespace XlsxToLua
{
    public class TableAnalyzeHelper
    {
            ...
            while (curColumnIndex < dt.Columns.Count)
            {
                ...
                else
                {
                    ...
                    if (oneField != null)
                    {
                        ...
                        {
                            errorString = _GetTableAnalyzeErrorString(tableName, nextColumnIndex) + string.Format("表格中存在字段名同为{0}的字段，分别位于第{1}列和第{2}列",
                                oneField.FieldName, Utils.GetExcelColumnName(fieldNames[oneField.FieldName] + 1),
                                Utils.GetExcelColumnName(oneField.ColumnSeq + 1));
                            return null;
                        }
                        else
                        {
        ...
        /// </summary>
        private static FieldInfo _AnalyzeOneField(DataTable dt, TableInfo tableInfo, int columnIndex, FieldInfo parentField,
            out int nextFieldColumnIndex, out string errorString)
        {
            // 判断列号是否越界
            ...
            // 字段描述中的换行全替换为空格
            fieldInfo.Desc = dt.Rows[AppValues.DATA_FIELD_DESC_INDEX][columnIndex].ToString().Trim().Replace(System.Environment.NewLine, " ")
                .Replace('\n', ' ').Replace('\r', ' ').Replace('\t', ' ');
            // 字段所在列号
            fieldInfo.ColumnSeq = columnIndex;
            // 检查规则字符串
            string checkRuleString = dt.Rows[AppValues.DATA_FIELD_CHECK_RULE_INDEX][columnIndex].ToString().Trim()
            ...
            if (parentField != null && parentField.DataType == DataType.Array)
            {
                ...
                if (parentField.ArrayChildDataType == DataType.Array)
                {
                    ...
                    {
                        errorString = string.Format("父array声明的子元素类型为array，但第{0}列所填的类型为{1}", Utils.GetExcelColumnName(columnIndex + 1),
                    ...
                    int defineCount;
                    _GetArrayChildDefine(parentField.ArrayChildDataTypeString, out defineDataTypeString, out defineDataType, out defineCount,
                        out errorString);
                    if (childDataType != defineDataType)
                    {
                        errorString = string.Format("父array的array类型子元素所声明的子元素为{0}型，但你填写的经过解析为{1}型", defineDataType.ToString(),
                            childDataType.ToString());
                        nextFieldColumnIndex = columnIndex + 1;
                        return null;
                    }
                ...
                else if (parentField.ArrayChildDataType == DataType.Dict)
                {
                    ...
                    else
                    {
                        ...
                        {
                            errorString = string.Format("父array声明的子元素类型为dict，但第{0}列所填的类型为{1}", Utils.GetExcelColumnName(columnIndex + 1),
                                inputDataType.ToString());
                            nextFieldColumnIndex = columnIndex + 1;
                            return null;
                        }
            ...
            else if (string.IsNullOrEmpty(inputFieldName))
            {
                ...
                {
                    Utils.LogWarning(string.Format("警告：第{0}列未填写变量名，仅填写了导出数据库信息，将不会进行lua、csv、json等客户端形式导出",
                        Utils.GetExcelColumnName(fieldInfo.ColumnSeq + 1)));
        ...
        private static bool _AnalyzeIntType(FieldInfo fieldInfo, TableInfo tableInfo, DataTable dt, int columnIndex, FieldInfo parentField,
            out int nextFieldColumnIndex, out string errorString)
        {
        ...
        private static bool _AnalyzeLongType(FieldInfo fieldInfo, TableInfo tableInfo, DataTable dt, int columnIndex, FieldInfo parentField,
            out int nextFieldColumnIndex, out string errorString)
        {
            fieldInfo.Data = new List<object>();
            // 记录非法数据的行号以及数据值（key：行号， value：数据值）
        ...
        private static bool _AnalyzeFloatType(FieldInfo fieldInfo, TableInfo tableInfo, DataTable dt, int columnIndex, FieldInfo parentField,
            out int nextFieldColumnIndex, out string errorString)
        {
            fieldInfo.Data = new List<object>();
            // 记录非法数据的行号以及数据值（key：行号， value：数据值）
        ...
        private static bool _AnalyzeBoolType(FieldInfo fieldInfo, TableInfo tableInfo, DataTable dt, int columnIndex, FieldInfo parentField,
            out int nextFieldColumnIndex, out string errorString)
        {
            fieldInfo.Data = new List<object>();
            // 记录非法数据的行号以及数据值（key：行号， value：数据值）
        ...
        private static bool _AnalyzeStringType(FieldInfo fieldInfo, TableInfo tableInfo, DataTable dt, int columnIndex, FieldInfo parentField,
            out int nextFieldColumnIndex, out string errorString)
        {
            // 检查string型字段数据格式声明是否正确
            if (!"string(trim)".Equals(fieldInfo.DataTypeString) && !"string".Equals(fieldInfo.DataTypeString))
            {
                errorString = string.Format("错误：string型字段定义非法，若要自动去除输入字符串的首尾空白字符请将数据类型声明为\"string(trim)\"，否则声明为\"string\"，而你输入的为\"{0}\"",
        ...
        private static bool _AnalyzeLangType(FieldInfo fieldInfo, TableInfo tableInfo, DataTable dt, int columnIndex, FieldInfo parentField,
            out int nextFieldColumnIndex, out string errorString)
        {
            ...
            if (!fieldInfo.DataTypeString.Equals("lang", StringComparison.CurrentCultureIgnoreCase))
            {
                ...
                if (replaceInfo.Count < 1)
                    Utils.LogWarning(string.Format("警告：表格{0}中的Lang型字段{1}（列号：{2}）进行统一配置但使用的是完全相同的key名（{3}），请确定是否要这样设置", tableInfo.TableName,
                        fieldInfo.FieldName, Utils.GetExcelColumnName(fieldInfo.ColumnSeq + 1), configString));
            }
        ...
        /// </summary>
        private static Dictionary<string, List<object>> _GetLangKeyReplaceInfo(string defineString, TableInfo tableInfo, out string configString,
        ...
        /// 解析date型数据的定义
        /// </summary>
        private static bool _AnalyzeDateType(FieldInfo fieldInfo, TableInfo tableInfo, DataTable dt, int columnIndex, FieldInfo parentField,
            out int nextFieldColumnIndex, out string errorString)
        {
            ...
            if (!fieldInfo.ExtraParam.ContainsKey(AppValues.TABLE_INFO_EXTRA_PARAM_KEY_DATE_INPUT_FORMAT))
            {
                ...
                {
                    errorString = string.Format("未声明date型的输入格式，在config配置文件中也未定义名为\"{0}\"的默认格式配置项",
                        AppValues.APP_CONFIG_KEY_DEFAULT_DATE_INPUT_FORMAT);
                    nextFieldColumnIndex = columnIndex + 1;
                    return false;
                }
                else
            ...
            if (!fieldInfo.ExtraParam.ContainsKey(AppValues.TABLE_INFO_EXTRA_PARAM_KEY_DATE_TO_LUA_FORMAT))
            {
                ...
                {
                    errorString = string.Format("未声明date型导出至lua文件的格式，在config配置文件中也未定义名为\"{0}\"的默认格式配置项",
                        AppValues.APP_CONFIG_KEY_DEFAULT_DATE_TO_LUA_FORMAT);
                    nextFieldColumnIndex = columnIndex + 1;
                    return false;
                }
                else
            ...
            if (!fieldInfo.ExtraParam.ContainsKey(AppValues.TABLE_INFO_EXTRA_PARAM_KEY_DATE_TO_DATABASE_FORMAT))
            {
                ...
                {
                    errorString = string.Format("未声明date型导出至MySQL数据库的格式，在config配置文件中也未定义名为\"{0}\"的默认格式配置项",
                        AppValues.APP_CONFIG_KEY_DEFAULT_DATE_TO_DATABASE_FORMAT);
                    nextFieldColumnIndex = columnIndex + 1;
                    return false;
                }
                else
        ...
        /// 解析time型数据的定义
        /// </summary>
        private static bool _AnalyzeTimeType(FieldInfo fieldInfo, TableInfo tableInfo, DataTable dt, int columnIndex, FieldInfo parentField,
            out int nextFieldColumnIndex, out string errorString)
        {
            ...
            if (!fieldInfo.ExtraParam.ContainsKey(AppValues.TABLE_INFO_EXTRA_PARAM_KEY_TIME_INPUT_FORMAT))
            {
                ...
                {
                    errorString = string.Format("未声明time型的输入格式，在config配置文件中也未定义名为\"{0}\"的默认格式配置项",
                        AppValues.APP_CONFIG_KEY_DEFAULT_TIME_INPUT_FORMAT);
                    nextFieldColumnIndex = columnIndex + 1;
                    return false;
                }
                else
            ...
            if (!fieldInfo.ExtraParam.ContainsKey(AppValues.TABLE_INFO_EXTRA_PARAM_KEY_TIME_TO_LUA_FORMAT))
            {
                ...
                {
                    errorString = string.Format("未声明time型导出至lua文件的格式，在config配置文件中也未定义名为\"{0}\"的默认格式配置项",
                        AppValues.APP_CONFIG_KEY_DEFAULT_TIME_TO_LUA_FORMAT);
                    nextFieldColumnIndex = columnIndex + 1;
                    return false;
                }
                else
            ...
            if (!fieldInfo.ExtraParam.ContainsKey(AppValues.TABLE_INFO_EXTRA_PARAM_KEY_TIME_TO_DATABASE_FORMAT))
            {
                ...
                {
                    errorString = string.Format("未声明time型导出至MySQL数据库的格式，在config配置文件中也未定义名为\"{0}\"的默认格式配置项",
                        AppValues.APP_CONFIG_KEY_DEFAULT_TIME_TO_DATABASE_FORMAT);
                    nextFieldColumnIndex = columnIndex + 1;
                    return false;
                }
                else
                    ...
                    else
                    {
                        ...
                        else
                        {
                            try
                            {
                                ...
                                fieldInfo.Data.Add(new DateTime(AppValues.REFERENCE_DATE.Year, AppValues.REFERENCE_DATE.Month,
                                    AppValues.REFERENCE_DATE.Day, tempDateTime.Hour, tempDateTime.Minute, tempDateTime.Second));
                            }
                            catch
                            {
                                invalidInfo.Add(row, inputData);
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                DateTime dateTime = new DateTime(AppValues.REFERENCE_DATE.Year, AppValues.REFERENCE_DATE.Month,
                                    AppValues.REFERENCE_DATE.Day, hour, minute, remainingSecond);
                                fieldInfo.Data.Add(dateTime);
                            }
                        }
                    }
        ...
        /// </summary>
        private static bool _AnalyzeJsonType(FieldInfo fieldInfo, TableInfo tableInfo, DataTable dt, int columnIndex, FieldInfo parentField,
            out int nextFieldColumnIndex, out string errorString)
        {
            StringBuilder errorStringBuilder = new StringBuilder();
            fieldInfo.Data = new List<object>();
        ...
        /// </summary>
        private static bool _AnalyzeTableStringType(FieldInfo fieldInfo, TableInfo tableInfo, DataTable dt, int columnIndex, FieldInfo parentField,
            out int nextFieldColumnIndex, out string errorString)
        {
        ...
        private static bool _AnalyzeMapStringType(FieldInfo fieldInfo, TableInfo tableInfo, DataTable dt, int columnIndex, FieldInfo parentField,
            out int nextFieldColumnIndex, out string errorString)
        {
            StringBuilder errorStringBuilder = new StringBuilder();
            ...
            else
            {
                ...
                else if (keyFormatString.StartsWith("#"))
                {
                    ...
                    if (!(formatDefine.KeyDefine.DataInIndexDefine.DataType == DataType.Int ||
                          formatDefine.KeyDefine.DataInIndexDefine.DataType == DataType.Long ||
                          formatDefine.KeyDefine.DataInIndexDefine.DataType == DataType.String))
                    {
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < tableElementDefine.Length; ++i)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    errorString = string.Format("table类型的第{0}个与第{1}个子元素均为相同的key（{2}）", tableKeys[oneTableElementDefine.KeyName] + 1,
                                        i + 1, oneTableElementDefine.KeyName);
                                    return formatDefine;
                                }
                                else
                                {
        ...
        private static DataInIndexDefine _GetDataInIndexDefine(string defineString, out string errorString)
        {
            ...
            else
            {
                ...
                if (!(dataInIndexDefine.DataType == DataType.Int || dataInIndexDefine.DataType == DataType.Long ||
                      dataInIndexDefine.DataType == DataType.Float || dataInIndexDefine.DataType == DataType.Bool ||
                      dataInIndexDefine.DataType == DataType.String || dataInIndexDefine.DataType == DataType.Lang))
                {
        ...
        private static bool _AnalyzeArrayType(FieldInfo fieldInfo, TableInfo tableInfo, DataTable dt, int columnIndex, FieldInfo parentField,
            out int nextFieldColumnIndex, out string errorString)
        {
                        ...
                        if ((bool) fieldInfo.ChildField[j].Data[i] == true)
                        {
                            if (invalidDataIndex != 0)
                            {
                                errorString = string.Format(
                                    "array的子元素为array或dict类型时，当前面的子元素用-1标识为无效后，后面的数据也必须声明为无效的。而第{0}行第{1}个子元素声明为无效，而后面第{2}个子元素却有效\n",
        ...
        /// </summary>
        private static bool _GetArrayChildDefine(string defineString, out string childDataTypeString, out DataType childDataType, out int childCount,
            out string errorString)
        {
        ...
        private static bool _AnalyzeDictType(FieldInfo fieldInfo, TableInfo tableInfo, DataTable dt, int columnIndex, FieldInfo parentField,
            out int nextFieldColumnIndex, out string errorString)
        {
            // dict或array集合类型中，如果定义列中的值填-1代表这行数据的该字段不生效，Data中用true和false代表该集合字段的某行数据是否生效
            fieldInfo.Data = _GetValidInfoForSetData(dt, columnIndex, fieldInfo.TableName);
        ...
        private static List<object> _GetValidInfoForSetData(DataTable dt, int columnIndex, string tableName)
        {
            ...
            for (int row = AppValues.DATA_FIELD_DATA_START_INDEX; row < dt.Rows.Count; ++row)
            {
                ...
                {
                    Utils.LogWarning(string.Format("警告：表格{0}的第{1}列的第{2}行数据有误，array或dict类型中若某行不需要此数据请填-1，否则留空，你填写的为\"{3}\"，本工具按有效值进行处理，但请按规范更正",
                        tableName, Utils.GetExcelColumnName(columnIndex + 1), row + 1, inputData));
                    validInfo.Add(true);
                }
            }

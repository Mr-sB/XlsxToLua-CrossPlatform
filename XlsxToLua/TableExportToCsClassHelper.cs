    ...
    public class TableExportToCsClassHelper
    {
        ...
        public static bool ExportTableToCsClass(TableInfo tableInfo, out string errorString)
        {
            ...
            if (AppValues.ExportCsClassUsing == null || !AppValues.ExportCsClassUsing.Contains(_SET_DATA_TYPE_USING_STRING))
            {
                ...
                {
                    if (fieldInfo.DataType == DataType.Array || fieldInfo.DataType == DataType.Dict || fieldInfo.DataType == DataType.Json ||
                        fieldInfo.DataType == DataType.TableString)
                    {
                ...
                case DataType.TableString:
                {
                        ...
                        switch (fieldInfo.TableStringFormatDefine.ValueDefine.ValueType)
                        {
                            ...
                            case TableStringValueType.DataInIndex:
                            {
                                string valueDataTypeString =
                                    _GetCsClassTableStringDataType(fieldInfo.TableStringFormatDefine.ValueDefine.DataInIndexDefine.DataType);
                    ...
                    else if (fieldInfo.TableStringFormatDefine.KeyDefine.KeyType == TableStringKeyType.DataInIndex)
                    {
                        ...
                        string keyDataTypeString =
                            _GetCsClassTableStringDataType(fieldInfo.TableStringFormatDefine.KeyDefine.DataInIndexDefine.DataType);
                            ...
                            case TableStringValueType.DataInIndex:
                            {
                                ...
                                if (valueDataTypeString == null)
                                    Utils.LogErrorAndExit(
                                        "用_GetCsClassFieldDefine函数导出csv对应C#类文件中tableString型字段的dataInIndex型key的dataInIndex型value的数据类型非法");
                                else
                                    stringBuilder.Append(valueDataTypeString);
